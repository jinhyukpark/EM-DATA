...
export default function DisclosureData() {
  ...
  const searchFields = [
    ...
    { value: "corpCode", label: "Corp Code" },
    { value: "corpName", label: "Corp Name" },
    { value: "reportNm", label: "Report Name" },
    { value: "rceptNo", label: "Receipt No" },
    { value: "flrNm", label: "Filer Name" },
    { value: "rceptDt", label: "Receipt Date" },
  ];

  const filteredData = disclosureData.filter((item: any) => {
                      ...
                      <PopoverContent className="w-56 p-0 bg-slate-950 border-slate-800" align="end">
                        <div className="p-3 border-b border-slate-800 bg-slate-900/50">
                           <div className="flex items-center justify-between">
                            ...
                            <span className="text-xs text-slate-400">{searchFields.filter(f => f.value !== 'all').length} fields</span>
                          </div>
                        </div>
                        <div className="p-1.5 space-y-0.5 max-h-[300px] overflow-y-auto custom-scrollbar">
                          {searchFields.filter(f => f.value !== 'all').map((field) => (
                            <div 
                              key={field.value} 
              ...
              <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-slate-50/50 border-b border-slate-100">
                      <tr>
                        {(visibleColumns["corpCode"] !== false) && (
                          <ResizableHeader id="corpCode" label="Corp Code">
                            {renderColumnConfig("corpCode", "Corp Code")}
                          </ResizableHeader>
                        )}
                        {(visibleColumns["corpName"] !== false) && (
                          <ResizableHeader id="corpName" label="Corp Name">
                            {renderColumnConfig("corpName", "Corp Name")}
                          </ResizableHeader>
                        )}
                        {(visibleColumns["reportNm"] !== false) && (
                          <ResizableHeader id="reportNm" label="Report Name">
                            {renderColumnConfig("reportNm", "Report Name")}
                          </ResizableHeader>
                        )}
                        {(visibleColumns["rceptNo"] !== false) && (
                          <ResizableHeader id="rceptNo" label="Receipt No">
                            {renderColumnConfig("rceptNo", "Receipt No")}
                          </ResizableHeader>
                        )}
                        {(visibleColumns["flrNm"] !== false) && (
                          <ResizableHeader id="flrNm" label="Filer Name">
                            {renderColumnConfig("flrNm", "Filer Name")}
                          </ResizableHeader>
                        )}
                        {(visibleColumns["rceptDt"] !== false) && (
                          <ResizableHeader id="rceptDt" label="Receipt Date">
                            {renderColumnConfig("rceptDt", "Receipt Date")}
                          </ResizableHeader>
                        )}
                      </tr>
                    </thead>
                        ...
                        <tr>
                          <td colSpan={10} className="px-6 py-12 text-center text-slate-500">
                            <div className="flex flex-col items-center gap-2">
                              ...
                              <p>No disclosure data found matching your criteria.</p>
                            </div>
                          </td>
                        </tr>
                      )}
